...
import (
	"fmt"
	resolver2 "github.com/tilau2328/x/src/go/service/repo/go/package/adaptor/driver/resolver"
...
func NewRestorer() *Restorer {
	return &Restorer{Map: model.NewMap(), Fset: token.NewFileSet(), Resolver: resolver2.NewGuessResolver(), Path: "root"}
}
	...
	if added {
		...
		for _, spec := range blocks[0].Specs {
			...
			if strings.Contains(path, ".") && !foundDomainImport {
				spec.Decors().Before = model.EmptyLine
				spec.Decors().After = model.NewLine
				foundDomainImport = true
				continue
			}
			spec.Decors().Before = model.NewLine
			spec.Decors().After = model.NewLine
		}
...
func (r *FileRestorer) restoreIdent(n *model.Ident, parentName, parentField, parentFieldType string, allowDuplicate bool) ast.Node {
	...
	out.X = r.restoreNode(&model.Ident{Name: name}, "Selector", "X", "Expr", allowDuplicate).(ast.Expr)
	r.cursor += token.Pos(len(token.PERIOD.String()))
	r.applyDecs(out, "X", n.Decs.X, false)
	out.Sel = r.restoreNode(&model.Ident{Name: n.Name}, "Selector", "Sel", "Ident", allowDuplicate).(*ast.Ident)
...
func (r *FileRestorer) applyDecs(node ast.Node, name string, Decs model.Decs, end bool) {
	firstLine := true
