...
func NewStruct(props Struct) *builder.GenBuilder {
	return builder.NewStruct(props.Name, NewStructFields(props.Fields)...)
}
...
func NewBuilder(props Struct) []builder.DeclBuilder {
	...
	ret = append(ret, lo.Map(props.Fields, func(item StructField, _ int) builder.DeclBuilder {
		return builder.NewFunc(item.Names[0], NewFuncType(FuncType{
			In:  []MethodType{{Names: []string{"v"}, Type: item.Type}},
			Out: []MethodType{{Type: Ident{Name: name, Ptr: true}}},
		}), builder.Block(
...
func NewService(props Strategy) []builder.DeclBuilder {
	return []builder.DeclBuilder{
