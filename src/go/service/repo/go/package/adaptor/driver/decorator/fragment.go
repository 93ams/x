...
func (f *fileDecorator) addDecorationFragment(n ast.Node, name string, pos token.Pos) {
	f.fragments = append(f.fragments,
		&decorationFragment{nodeFragment: nodeFragment{Node: n, Pos: token.Pos(f.cursor)}, Name: name})
}
func (f *fileDecorator) addTokenFragment(n ast.Node, t token.Token, pos token.Pos) {
	...
	f.fragments = append(f.fragments,
		&tokenFragment{nodeFragment: nodeFragment{Node: n, Pos: token.Pos(f.cursor)}, Token: t})
...
func (f *fileDecorator) addStringFragment(n ast.Node, s string, pos token.Pos) {
	...
	f.fragments = append(f.fragments,
		&stringFragment{nodeFragment: nodeFragment{Node: n, Pos: token.Pos(f.cursor)}, String: s})
	f.cursor += len(s)
}
func (f *fileDecorator) addBadFragment(n ast.Node, pos token.Pos, length int) {
	...
	f.fragments = append(f.fragments,
		&badFragment{nodeFragment: nodeFragment{Node: n, Pos: token.Pos(f.cursor)}, Length: length})
	f.cursor += length
}
func (f *fileDecorator) addCommentFragment(text string, pos token.Pos) {
	f.fragments = append(f.fragments, &commentFragment{decFragment: decFragment{Pos: pos}, Text: text})
}
func (f *fileDecorator) addNewlineFragment(pos token.Pos, empty bool) {
	f.fragments = append(f.fragments, &newlineFragment{decFragment: decFragment{Pos: pos}, Empty: empty})
}
	...
	for i, frag := range f.fragments {
		...
		case *decorationFragment:
			...
			if len(endFrags) > 0 {
				...
				if nl {
					f.attachToDecoration(endFrags[0:len(endFrags)-1], f.Decs, frag)
				} else {
					f.attachToDecoration(endFrags, f.Decs, frag)
				}
			}
			if len(nextFrags) > 0 && next != nil {
				...
				if (nextStmt || nextDecl) && nextStart == start {
					f.attachToDecoration(nextFrags, f.Decs, next)
				}
			}
		case *commentFragment:
			...
			f.attachToDecoration(frags, f.Decs, dec)
		}
	}
	for i, frag := range f.fragments {
		switch frag := frag.(type) {
		case *newlineFragment:
			...
			appendNewLine(f.Decs, dec.Node, dec.Name, frag.Empty)
		}
	}
	return
}
...
func (f *fileDecorator) attachToDecoration(frags []fragment, Decs map[ast.Node]map[string][]string, dec *decorationFragment) {
	for _, fr := range frags {
		switch fr := fr.(type) {
		case *commentFragment:
			appendDecoration(Decs, dec.Node, dec.Name, fr.Text)
			fr.Attached = dec
		case *newlineFragment:
			appendNewLine(Decs, dec.Node, dec.Name, fr.Empty)
			fr.Attached = dec
		}
	}
}
...
type fragment interface {
	Position() token.Pos
	Newline() bool // True if the nodeFragment ends in a newline ("\n" or "//...")
}
...
type (
	...
	newlineFragment struct {
		Empty bool
		decFragment
	}
	decorationFragment struct {
		nodeFragment
		Name string
	}
)
...
func (f *fileDecorator) addNodeFragments(n ast.Node) {
	...
	case *ast.FuncDecl:
		...
		if n.Recv != nil {
			f.addDecorationFragment(n, "RecvChan", token.NoPos)
		}
		if n.Name != nil {
			f.addNodeFragments(n.Name)
		}
