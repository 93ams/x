...
import (
	"fmt"
	"github.com/tilau2328/x/src/go/service/repo/go/package/adaptor/driver/resolver"
...
type (
	...
	fileDecorator struct {
		*Decorator
		...
		Decs          map[ast.Node]map[string][]string
	}
)
...
func (d *Decorator) newFileDecorator() *fileDecorator {
...
func (f *fileDecorator) decorateSelectorExpr(_ ast.Node, _, _, _ string, n *ast.SelectorExpr) (model.Node, error) {
	if f.Resolver == nil {
		return nil, nil
	}
	...
	if decs, ok := f.Decs[n]; ok {
		nStart = decs["Start"]
		nX = decs["X"]
		nEnd = decs["End"]
	}
	if decs, ok := f.Decs[n.X]; ok {
		xStart = decs["Start"]
		xEnd = decs["End"]
	}
	if decs, ok := f.Decs[n.Sel]; ok {
		sStart = decs["Start"]
		sEnd = decs["End"]
	}
	if iStart := mergeDecs(nStart, xBefore, xStart); len(iStart) > 0 {
		out.Decs.Start.Append(iStart...)
	}
	if iX := mergeDecs(xEnd, xAfter, nX, sBefore, sStart); len(iX) > 0 {
		out.Decs.X.Append(iX...)
	}
	if iEnd := mergeDecs(sEnd, sAfter, nEnd); len(iEnd) > 0 {
...
func (f *fileDecorator) decorateObject(o *ast.Object) (*model.Object, error) {
	...
	out := &model.Object{}
	f.Dst.Objects[o], f.Ast.Objects[out] = out, o
...
func (f *fileDecorator) decorateScope(s *ast.Scope) (*model.Scope, error) {
	...
	out := &model.Scope{}
	f.Dst.Scopes[s], f.Ast.Scopes[out] = out, s
...
func mergeDecs(DecsOrLineSpace ...interface{}) []string {
	var endsWithNewLine bool
	var out []string
	for _, v := range DecsOrLineSpace {
		switch v := v.(type) {
		case nil:
		case []string:
			if len(v) == 0 {
