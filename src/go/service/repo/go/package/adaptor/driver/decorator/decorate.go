...
func (f *fileDecorator) decorateNode(parent ast.Node, parentName, parentField, parentFieldType string, n ast.Node) (model.Node, error) {
	...
	case *ast.ArrayType:
		out := &model.Array{}
		f.Dst.Nodes[n], f.Ast.Nodes[out] = out, n
		out.Decs.Before, out.Decs.After = f.before[n], f.after[n]
		if n.Len != nil {
	...
	case *ast.FuncDecl:
		...
		f.Dst.Nodes[n], f.Ast.Nodes[out] = out, n
		out.Decs.Before, out.Decs.After = f.before[n], f.after[n]
		f.Dst.Nodes[n.Type], f.Ast.Nodes[out.Type] = out.Type, n.Type
		if n.Recv != nil {
			child, err := f.decorateNode(n, "Func", "RecvChan", "FieldList", n.Recv)
			if err != nil {
				return nil, err
			}
